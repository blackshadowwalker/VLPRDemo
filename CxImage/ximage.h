...
/** \addtogroup Constructors */ //@{
	...
	CxImage(uint32_t dwWidth, uint32_t dwHeight, uint32_t wBpp, uint8_t* pLargeBuff,uint32_t imagetype = 0);
...
/** \addtogroup Initialization */ //@{
	void*	Create(uint32_t dwWidth, uint32_t dwHeight, uint32_t wBpp, uint32_t imagetype = 0, uint8_t* pucExternBuff = NULL);
	bool	Destroy();
	bool	DestroyFrames();
	...
	bool    SetImageData(uint8_t*  pImageBuff,uint32_t imagesize);
...
/** \addtogroup Painting */ //@{
	...
	bool	CreateFromHBITMAP(HBITMAP hbmp, HPALETTE hpal=0, bool bTransparency = false,unsigned char* pImageBuff = NULL);	//Windows resource
	...
	int32_t	DrawString(HDC hdc, int32_t x, int32_t y, const TCHAR* text, RGBQUAD color, const TCHAR* font, int32_t lSize=0, int32_t lWeight=400, uint8_t bItalic=0, uint8_t bUnderline=0, bool bSetAlpha=false,unsigned char* pExterBuff = NULL);

	int32_t	DrawStringArray(HDC hdc, stDrawTextArray pDrawText,int nCount, RGBQUAD color, const TCHAR* font, int32_t lSize=0, int32_t lWeight=400, uint8_t bItalic=0, uint8_t bUnderline=0, bool bSetAlpha=false);
...
/** \addtogroup Encode */ //@{
...
#endif
	// For UNICODE support: char -> TCHAR
	bool Save(const TCHAR* filename, uint32_t imagetype,bool isBGR = false);
	...
	bool Encode(uint8_t * buffer, int32_t buffsize,int32_t& encodesize,uint32_t imagetype);
...
/** \addtogroup Transformations */ //@{
	...
	bool ResampleSimple(int32_t newx, int32_t newy, CxImage* iDst,bool isFlip=false);


	bool Resample(int32_t newx, int32_t newy, int32_t mode = 1, CxImage* iDst = NULL,unsigned char* pExternBuff = NULL);
...
/** \addtogroup ColorSpace */ //@{
	bool ConvertYUV(uint8_t* pY,uint32_t dwYLen,uint8_t* pU,uint32_t dwULen,uint8_t* pV,uint32_t dwVLen);
