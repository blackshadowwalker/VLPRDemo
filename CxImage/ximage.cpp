...
CxImage::CxImage(uint32_t dwWidth, uint32_t dwHeight, uint32_t wBpp, uint8_t* pexternLargeBuff,uint32_t imagetype )
{
	Startup(imagetype);
	Create(dwWidth,dwHeight,wBpp,imagetype,pexternLargeBuff);
}
 ...
 */
void* CxImage::Create(uint32_t dwWidth, uint32_t dwHeight, uint32_t wBpp, uint32_t imagetype, uint8_t* pucExternBuff )
{
...
bool CxImage::Transfer(CxImage &from, bool bTransferFrames /*=true*/)
{
	...
	bUseExterBuff = from.bUseExterBuff;
	pDib = from.pDib;
...
bool   CxImage::SetImageData(uint8_t*  pImageBuff,uint32_t dwImageSize)
{
	if(head.biSizeImage != 0 && info.pImage != 0 && dwImageSize == head.biSizeImage )
	{
		memcpy(info.pImage,pImageBuff,head.biSizeImage);
		return true;
	}	
	return false;
}
...
//2013.4.22  xuehz add watermark at jpg tail in teleframe

bool cxValidJpgWaterMark(const TCHAR * filename)
{
	FILE* hFile;
    char cDebugBuff[512] = {0};
	int nFileSize = 0;
	int nCalcMd5Len = 0;
	unsigned char cFileOrgMark[WATERMARK_LEN] = {0};
	unsigned char cWaterMarkTail[WATERMARK_LEN] = {0};
	unsigned char cBuff[1024+8];
...
//	sprintf(cDebugBuff,"file len %d \n",nFileSize);
//	OutputDebugString(cDebugBuff);

	nFileSize = nFileSize - WATERMARK_LEN;//先取得jpg的真正长度

//	sprintf(cDebugBuff,"file real len %d \n",nFileSize);
//	OutputDebugString(cDebugBuff);


	cWaterMarkTail[0] = 0xFF;
	cWaterMarkTail[1] = 0xFE;
...
//   	OutputDebugString(cDebugBuff);
		...
		XMD5_CTX md5cxt;
		unsigned char cMd5Data[MD5_DIGEST_SIZE];
		md5_init(&md5cxt);
		filex.Seek(0,SEEK_SET);
		for(m=0; m<nCnt; m++)
		{
			filex.Read(cBuff,1024,1);
	...
	filex.Seek(-WATERMARK_LEN,SEEK_END);
	filex.Read(cFileOrgMark,WATERMARK_LEN,1);

	filex.Close(); 
	if(0 == memcmp(cFileOrgMark,cWaterMarkTail,WATERMARK_LEN))
	{
		fclose(hFile);
		return true;
	}
