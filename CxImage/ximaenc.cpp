...
bool CxImage::ConvertYUV(uint8_t* pY,uint32_t dwYLen,uint8_t* pU,uint32_t dwULen,uint8_t* pV,uint32_t dwVLen)
{
	int nWidth,nHeight,widthstep;
	int y,x;
	...
	nWidth = GetWidth();
	nHeight = GetHeight();
	widthstep = (((nWidth * 3 * (8 & ~0x80000000) + 7)/8)+ 1 - 1) & (~(1 - 1)); 

	unsigned char *uc;
	for(y=0;y<nHeight;y++) {
		uc = (unsigned char *)(info.pImage+widthstep*y);
		for(x=0;x<nWidth;x++) {
			//			picture->data[0][y * picture->linesize[0] + x] = uc[x*3+2]*0.257+uc[x*3+1]*0.504+uc[x*3]*0.098+16;
			pY[y *dwYLen+ x] = uc[x*3+2]/4+uc[x*3+1]/2+uc[x*3]/10+16;
		}
	}
	/* Cb and Cr */
	for(y=0;y<nHeight/2;y++) {
		uc = (unsigned char *)(info.pImage+widthstep*y*2);
		for(x=0;x<nWidth/2;x++) {
			//picture->data[1][y * picture->linesize[1] + x] = uc[x*6+2]*-0.148+uc[x*6+1]*-0.291+uc[x*6]*0.439+128;
			//picture->data[2][y * picture->linesize[2] + x] = uc[x*6+2]*0.439+uc[x*6+1]*-0.368+uc[x*6]*-0.071+128;
			pU[y * dwULen + x] = (-uc[x*6+2]*3-uc[x*6+1]*6+uc[x*6]*9)/20+128;
			pV[y * dwVLen + x] = (uc[x*6+2]*45-uc[x*6+1]*40-uc[x*6]*7)/100+128;
		}
	}
 ...
 */
bool CxImage::Save(const TCHAR * filename, uint32_t imagetype,bool isBGR)
{
	FILE* hFile;	//file handle to write the image
	int nFileSize = 0;
	char cDebugBuff[512] = {0};
	int nCalcMd5Len = 0;
	unsigned char cWaterMarkTail[WATERMARK_LEN] = {0};
	unsigned char cBuff[1024+8] = {0};

#ifdef WIN32
	if ((hFile=_tfopen(filename,_T("wb")))==NULL) 
	{
		sprintf(info.szLastError,"JPEG fopen %s failure",filename);
	...
	Sleep(1);
#ifdef WIN32
	if ((hFile=_tfopen(filename,_T("ab+")))==NULL)  
	{
		sprintf(info.szLastError,"JPEG fopen 2 %s failure",filename);
		return false;	// For UNICODE support
	}
#else
	...
	CxIOFile  filex(hFile);
	XMD5_CTX md5cxt;
	unsigned char cMd5Data[MD5_DIGEST_SIZE] = {0};

	nFileSize = filex.Size();




	cWaterMarkTail[0] = 0xFF;
	cWaterMarkTail[1] = M_COM;
	cWaterMarkTail[3] = WATERMARK_LEN-2;

	if(nFileSize > 1024*4)
	{
		int  nCnt = 0,m = 0;
		nCalcMd5Len = (nFileSize-2048) - (nFileSize-2048)%1024;

	//	sprintf(cDebugBuff,"nCalcMd5Len len %d \n",nCalcMd5Len);
	//	OutputDebugString(cDebugBuff);

		nCnt = nCalcMd5Len/1024;

	//	sprintf(cDebugBuff,"nCnt  %d \n",nCnt);
	//	OutputDebugString(cDebugBuff);


		md5_init(&md5cxt);
		filex.Seek(0,SEEK_SET);
		for(m=0; m<nCnt; m++)
		{
			if(filex.Read(cBuff,1024,1) > 0)
			{
				md5_update(&md5cxt,&cBuff[0],1024);
			}
			else
			{

				sprintf(cDebugBuff,"err  %d \n",::GetLastError());
			}

		}
		md5_final(&md5cxt,&(cMd5Data[0]));
		memcpy(&cWaterMarkTail[4],&(cMd5Data[0]),MD5_DIGEST_SIZE);
	}


	filex.Seek(0,SEEK_END);

	filex.Write(cWaterMarkTail,WATERMARK_LEN,1);
	filex.Close();
...
bool CxImage::Encode(uint8_t * buffer, int32_t buffsize,int32_t& encodesize, uint32_t imagetype)
{
	if (buffer == NULL || buffsize <=0 ){
		strcpy(info.szLastError,"the buffer must be empty");
		return false;
	}
	CxMemFile file(buffer,buffsize);
	file.Open();
	if(Encode(&file,imagetype)){
		buffer=file.GetBuffer();
