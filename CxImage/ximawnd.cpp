 ...
 * \author []; changes [brunom]
 */
bool CxImage::CreateFromHBITMAP(HBITMAP hbmp, HPALETTE hpal, bool bTransparency, unsigned char* pImageBuff)
{
	if (!Destroy())
		return false;
	...
	if (hbmp) { 
		...
		{
			// create the image
			if (!Create(bm.bmWidth, bm.bmHeight, bm.bmBitsPixel, 0,pImageBuff))
				return false;
...
int32_t CxImage::DrawString(HDC hdc, int32_t x, int32_t y, const TCHAR* text, RGBQUAD color, const TCHAR* font, int32_t lSize, int32_t lWeight, uint8_t bItalic, uint8_t bUnderline, bool bSetAlpha,unsigned char* pExterBuff)
//int32_t CxImage::DrawString(HDC hdc, int32_t x, int32_t y, const char* text, RGBQUAD color, const char* font, int32_t lSize, int32_t lWeight, uint8_t bItalic, uint8_t bUnderline, bool bSetAlpha)
{
	if (IsValid())
	{
		...
		CxImage itext;
		itext.CreateFromHBITMAP(TmpBmp,0,false,pExterBuff);
...
void CxImage::InitTextInfo( CXTEXTINFO *txt )
{
    ...
    txt->lfont.lfCharSet       = GB2312_CHARSET; // just for Central-European users 
    ...
    // the text 
    _stprintf( txt->text, _T(" ")); // text use TCHAR mappings <Cesar M>
    txt->align = DT_CENTER;
    return;
}
