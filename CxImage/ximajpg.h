...
public:
	...
	static void InitDestination(j_compress_ptr cinfo)
	{
		...
		pDest->next_output_byte = pDest->m_cBuffer;
	...
	{
		CxFileJpg* pDest = (CxFileJpg*)cinfo->dest;
		if (pDest->m_pFile->Write(pDest->m_cBuffer,1,eBufSize)!=(size_t)eBufSize)
	...
	static void TermDestination(j_compress_ptr cinfo)
	{
		...
		if (datacount > 0) {
			if (!pDest->m_pFile->Write(pDest->m_cBuffer,1,datacount))
				ERREXIT(cinfo, JERR_FILE_WRITE);
		}
		pDest->m_pFile->Flush();
	...
	static boolean FillInputBuffer(j_decompress_ptr cinfo)
	{
		size_t nbytes;
		CxFileJpg* pSource = (CxFileJpg*)cinfo->src;
		nbytes = pSource->m_pFile->Read(pSource->m_cBuffer,1,eBufSize);
		if (nbytes <= 0){
			if (pSource->m_bStartOfFile)	//* Treat empty input file as fatal error 
				ERREXIT(cinfo, JERR_INPUT_EMPTY);
			WARNMS(cinfo, JWRN_JPEG_EOF);
			// Insert a fake EOI marker 
			pSource->m_cBuffer[0] = (JOCTET) 0xFF;
			pSource->m_cBuffer[1] = (JOCTET) JPEG_EOI;
			nbytes = 2;
		}
		pSource->next_input_byte = pSource->m_cBuffer;
