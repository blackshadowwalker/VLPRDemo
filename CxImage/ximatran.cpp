...
bool CxImage::ResampleSimple(int32_t newx, int32_t newy, CxImage* iDst,bool isFlip)
{
	register int  y = 0, x= 0,yflip = 0;
    if (newx==0 || newy==0) 
		return false;
	...
	register float xScale, yScale, fX, fY;
	...
	if(iDst)
	{
		for( y=0; y<newy; y++)
		{
			...
			for( x=0; x<newx; x++)
			{
				...
				{	
					yflip = newy-1-y;
					iDst->SetPixelColor(x,yflip,GetPixelColor((int32_t)fX,(int32_t)fY));  
				}
				else
				{
					iDst->SetPixelColor(x,y,GetPixelColor((int32_t)fX,(int32_t)fY));
				}
			}
		}
 ...
 */
bool CxImage::Resample(int32_t newx, int32_t newy, int32_t mode, CxImage* iDst,unsigned char* pExternBuff)
{
	...
	newImage.Create(newx,newy,head.biBitCount,GetType(),pExternBuff);
